...
class HiddenMarkovModel:
    def __init__(self):
        ...
        self.transition_prob = np.zeros((45, 45))                           # Struktury zawierające prawdopodobieństwa przejścia stanów i emisji obserwacji.
        ...
        for line in input_lines:
            if line != "\n":
                ...
                if expression in self.emission_list[tag_index]:
                    self.emission_list[tag_index][expression] += 1          # Jeżeli wyrażenie jest zawarte w słowniku, dodaj 1 do wartości definicji...
                else:
                    self.emission_list[tag_index][expression] = 1           # ...a jeśli nie, to stwórz wpis w słowniku odpowiadający wyrażeniu, i ustaw definicję na 1.
    ...
    def initialize_probabilities(self):
        for index, line in enumerate(self.transition_list):
            ...
            if incidence_sum == 0:                                          # Jeżeli stan nie ma przejść, bo nie występuje, to wypełnij wiersz zerami.
                ...
                new_row = np.array([value / incidence_sum for value in line])   # Lista ilorazów przejść do stanów 0,1,2,...,44 i sumy wszystkich przejść stanów.
            self.transition_prob[index] = new_row
        for index, line in enumerate(self.emission_list):
            incidence_sum = sum([value for key, value in line.items()])             # Suma wszystkich obserwacji (wyrażeń) danego stanu.
            if incidence_sum == 0:
